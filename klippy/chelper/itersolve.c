                          ...
                          , double abs_start, double abs_end)
{
    ...
    struct timepos last = { start, sk->commanded_pos };
    struct timepos old_guess = last, guess = last;
    int sdir = stepcompress_get_step_dir(sk->sc), is_dir_change=0, have_bound=0;
    double target = last.position + (sdir ? half_step : -half_step);
    double seek_time_delta = SEEK_TIME_RESET;
    double low_time = start, high_time = start + seek_time_delta;
    if (high_time > end)
        high_time = end;
    for (;;) {
        ...
        if (fabs(guess_dist) > .000000001) {
            ...
            if (!have_bound || high_time - low_time > .000000001) {
        ...
        int ret = stepcompress_append(sk->sc, sdir
                                      , m->print_time, guess.time);
        if (ret)
            return ret;
        // Reset bounds checking
        seek_time_delta = 1.5 * (guess.time - last.time);
        ...
        is_dir_change = have_bound = 0;
        high_time = guess.time + seek_time_delta;
        if (high_time > end)
            high_time = end;
        // Update last position and set next target
        last.position = target + (sdir ? half_step : -half_step);
        last.time = low_time = guess.time;
        target = last.position + (sdir ? half_step : -half_step);
    }
